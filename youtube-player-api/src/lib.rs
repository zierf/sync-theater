...
pub use wrapper::{PlayerOptions, PlayerVars, YtPlayer, YtPlayerEvents, YtPlayerState};
...
pub fn init_yt_api() -> Result<(), JsValue> {
    #[cfg(feature = "std")]
    ...
    let previous_ready_function =
        Reflect::get(&window, &to_value("onYouTubeIframeAPIReady").unwrap())?;
    ...
    let api_script = document.create_element("script")?;
    api_script.set_attribute("src", "https://www.youtube.com/player_api")?;
